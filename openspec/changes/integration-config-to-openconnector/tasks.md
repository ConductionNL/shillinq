...
- [ ] Resolve `design.md` §6.1 (the `bzk-sisa` vs `bzk-sisa-upload-2026` slug
...
- [ ] Confirm `design.md` §6.2 (the ADR-067 rule 2 declaration mechanism —
...
- [ ] For each of the 15 families in `design.md` §1's table, add a stub entry
...
- [ ] Update the file's `_meta.imported` timestamp per the existing
      convention.

## 2. Manifest collapse (REQ-ICO-002, REQ-ICO-004)
- [ ] Rewrite `src/manifest.d/external-adapters-w8.json`: remove all 15
...
- [ ] Convert the `ExternalConnections` menu group from a 16-child parent
      into a single leaf node carrying `route` directly (see `design.md` §3's
      drafted shape).
- [ ] `wc -c` the resulting fragment and confirm it lands near the ~1,000
      byte estimate in `design.md` §7 (informational — not a hard gate).
- [ ] Delete `src/views/external-adapters/ExternalAdapterDetail.vue` — no
      route references it after this change.

## 3. Backend — provisioning resolution (REQ-ICO-003)
- [ ] Add `resolveProvisioning(string $sourceSlug): array` to
...
- [ ] Resolve the OpenRegister object-service via DI (same pattern as the
...
- [ ] Extend `#index`'s JSON response with each row's `provisioning: {
...
- [ ] Decide (per `design.md` §6.3) whether `#show` (lines 506–519) stays as
...
- [ ] Implement whatever mechanism task 0's second pre-work item settled on,
      enumerating all 15 `sourceSlug` values as declared-unimplemented
      integrations.

## 5. Frontend roster component (REQ-ICO-002)
- [ ] Rewrite `src/views/external-adapters/ExternalAdaptersStatus.vue` (or
...
- [ ] Ensure the empty/error states (OR unavailable → all rows `unknown`,
      per REQ-ICO-003's third scenario) render without a page-level crash.

## 6. e2e (REQ-ICO-007)
- [ ] Replace `tests/e2e/external-adapters.spec.ts`,
...
- [ ] Assert: 15 rows render; every row's deep-link `href` is well-formed;
...
- [ ] Grep the final `tests/e2e/**` tree for any remaining
      `/external-adapters/<family-id>` path literal and remove it.
- [ ] Tag new Playwright tests with `@e2e integration-config-to-
      openconnector::<scenario-slug>` matching `spec.md`'s scenario ids
      exactly (gate-19 / `hydra-gate-e2e-coverage`).

## 7. Validation
- [ ] `openspec validate integration-config-to-openconnector --strict` —
      PASS.
- [ ] Run the roster page e2e locally against a real backend.
- [ ] `grep -rn "config.adapterId" src/manifest.d/` — zero matches after
      task 2.
- [ ] Confirm no `lib/Service/External/**` file, and no
