...
- [ ] Task 10: **DEFERRED** — extend `Counterparty` with `defaultMandateId`. The `Counterparty` schema is owned by `bookkeeping-accounts-receivable-core`, which is NOT yet merged (no Counterparty schema exists in any register.d fragment or the monolith). Lands when AR-core merges; the FK relationship is documented in `adr-000-data-model.md`.
- [ ] Task 11: **DEFERRED** — extend `Invoice` with `paymentMethod` / `directDebitMandateId` / `directDebitPreNotificationInvoiceId`. The `Invoice` schema is owned by the not-yet-merged `bookkeeping-accounts-receivable-core`. Lands when AR-core merges; FK documented in `adr-000-data-model.md`.
...
- [~] Task 17: pain.008 **validation gate partially implemented**: `lib/Validation/IbanValidator` (ISO 13616 mod-97) ships and the schema carries controlSum/collectionCount/pain008Xml. The XML **marshalling** (PmtInf blocking, CdtrSchmeId, XSD validation) is **DEFERRED** to the `bookkeeping-bank-connectors` stack per the proposal (consumes the contract, does not implement the marshaller).
- [ ] Task 18: **DEFERRED** — pain.002 ingestion is event-driven (design D6) and depends on the `bookkeeping-bank-connectors` delivery contract (webhook/poll) which is not yet merged. The batch/collection lifecycle transitions it drives are declared; the ingestion handler lands with bank-connectors.
- [ ] Task 19: **DEFERRED** — camt.054 reconciliation depends on bank-connectors delivery AND the not-yet-merged Invoice/journal entities (mark-paid + journal posting). The `RTransaction` schema + collection `refund` transition are declared; the handler lands with bank-connectors + AR-core.
...
- [x] Task 23: Audit-trail export implemented for the mandate dossier: `GET /api/v1/sepa-mandate/{mandateId}/audit-export` (`SepaAuditController` + `SepaAuditService`, IDOR-safe tenant scoping, `#[NoAdminRequired]`) → ZIP of mandate.json + collections.csv/json + r-transactions.json + pre-notifications.json + archived pain fragments. The **invoice** dossier endpoint and **journal-line** inclusion are **DEFERRED** (depend on the not-yet-merged Invoice/journal entities).
- [~] Task 24: pain-file archival governance **partially implemented**: `DirectDebitBatch.isArchived` retention flag (default true) + RBAC denies `delete` on `RTransaction`. The explicit `delete-with-retention-override` endpoint is **DEFERRED** to a follow-up (requires OR retention-policy hooks / live object deletion).
- [x] Task 25: 4 manifest navigation entries (`SEPA Mandates`, `Direct Debit Collections`, `Direct Debit Batches`, `Rejections & Returns`) under a new top-level `Incasso` menu group + their `type: index` / `type: detail` pages added to `src/manifest.json`; every menu `route` resolves to a page `id`; lifecycle action surface enabled on the mandate / collection / batch detail pages so the declared `x-openregister-lifecycle` transitions render (REQ-SDD-008, REQ-SDD-009). Restored after a fleet of intervening merges (bbv-compliance, year-end-close, pension-ias19, …) had stripped the entries; app version bumped 0.7.2→0.7.3 + manifest 1.3.11→1.3.12 so the immutable JS/JSON cache invalidates for any browser that already loaded the prior bundle.
