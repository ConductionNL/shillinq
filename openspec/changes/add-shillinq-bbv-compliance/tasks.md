...
- [x] Task 6: Declare the `BbvTaakveld` schema in `lib/Settings/shillinq_register.json` (taakveldCode, name, category, legalBasis, effectiveFrom, effectiveTo) per REQ-BBV-005; loaded from `bbv-taakvelden-2024.json` seed
...
- [x] Task 10: Extend the repair step under `lib/Repair/InitializeSettings.php` to import the BBV seeds for new `gemeente`/`provincie`/`waterschap` administrations only (skip for non-municipal admins); idempotent on re-run
- [x] Task 11: Add `Overheid > BBV-mapping` navigation + pages to `src/manifest.json` with `type: index` + `type: detail`, visibility predicate scoped to municipal administration types per REQ-BBV-007
