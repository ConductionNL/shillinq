...
- [ ] Task 1: Confirm no `inventory-cycle-count` capability spec already exists, no 
...
- [ ] Task 2: Author `specs/inventory-cycle-count/spec.md` with `Status: proposed` / 
...
- [ ] Task 4: Author `design.md` with context, goals, non-goals, decisions (D1: line-
...
- [ ] Task 5: Declare the `InventoryCycleCount` schema in `lib/Settings/shillinq_register.json` 
...
- [ ] Task 6: Declare the `InventoryCycleCountLine` schema in `lib/Settings/shillinq_register.json` 
...
- [ ] Task 7: Declare the `InventoryVarianceReason` schema in `lib/Settings/shillinq_register.json` 
...
- [ ] Task 8: Add `x-openregister-lifecycle` to `InventoryCycleCount` declaring all 
...
- [ ] Task 9: Implement variance threshold flagging on `InventoryCycleCountLine` via 
...
- [ ] Task 10: Implement variance adjustment posting on `InventoryCycleCount.posted → 
...
- [ ] Task 11: Implement partial-count filtering (location / category scope) as standard 
...
- [ ] Task 12: Document webhook endpoint shape (POST `/api/cycle-count/{countId}/count-
...
- [ ] Task 13: Add 3 manifest navigation entries (`Cycle Counts`, `Count Templates`, 
...
- [ ] Task 14: Declare `InventoryAdjustment` (stub) in `openspec/architecture/adr-000-
...
- [ ] Task 15: Update `openspec/architecture/adr-000-data-model.md` with full entries for 
