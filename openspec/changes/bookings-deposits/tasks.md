...
- [x] Task 17: PARTIAL — OpenConnector calls (`createPaymentIntent` / `getPaymentStatus` / `initiateRefund`) are declared as lifecycle/workflow connector actions and injected into `pollPending()` via a status-provider callable. **Adapter port now shipped**: dormant `DepositPaymentAdapterInterface` + `LogDepositPaymentAdapter` at `lib/Service/External/DepositPayment/` carry the `requestPayment` / `fetchStatus` / `initiateRefund` contract one layer above the existing `MolliePaymentAdapterInterface` so the lifecycle code never sees a Mollie-vs-Stripe branch. Live wiring still DEFERRED — the production binding delegates to MolliePaymentAdapterInterface (already wired) and projects Mollie state onto the DepositPayment lifecycle state. REQ-DP-001 upheld (no direct PSP SDK calls).
