...
### REQ-ARC-001: Records retention SHALL be enforced via OpenRegister's lifecycle retention abstraction, NOT reimplemented in shillinq
...
### REQ-ARC-002: Retention rules SHALL ship as a `RetentionRule` register seeded from `selectielijst-gemeenten-2020.json`
...
### REQ-ARC-003: Every shillinq schema SHALL declare its retention rule reference
...
### REQ-ARC-004: Retention rules with PII-anonymisation disposition SHALL be marked accordingly
...
### REQ-ARC-005: Retention overrides for administration-specific archiefverordening SHALL be operator-editable
...
### REQ-ARC-006: The audit-trail hash chain SHALL remain verifiable across retention dispositions
...
### REQ-ARC-007: Operators SHALL be able to query records nearing retention via a derived field
...
### REQ-ARC-008: A "records nearing retention" widget SHALL be declared via `x-openregister-widgets`
...
### REQ-ARC-009: Retention administration SHALL be reachable through the shillinq manifest navigation
...
### REQ-ARC-010: Audit trail of retention rule changes SHALL be consumed from OR's abstractions
