...
- [ ] Task 1: Confirm no `bookkeeping-quote-order-invoice` capability spec
...
- [ ] Task 2: Author `specs/bookkeeping-quote-order-invoice/spec.md` with
...
- [ ] Task 4: Author `design.md` with Reuse Analysis table, D1–D12 decisions
...
- [ ] Task 5: Declare the `Quote` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 6: Declare the `QuoteLine` schema in
...
- [ ] Task 7: Declare the `SalesOrder` schema in
...
- [ ] Task 8: Declare the `SalesOrderLine` schema in
  `lib/Settings/shillinq_register.json` with all REQ-QOI-004 fields
...
- [ ] Task 9: Declare the `Delivery` schema in
...
- [ ] Task 10: Declare the `Invoice` schema in
...
- [ ] Task 11: Declare the `InvoiceLine` schema in
...
- [ ] Task 12: Declare the `CreditNote` schema in
...
- [ ] Task 13: Declare the `PricingTier` schema in
...
- [ ] Task 14: Declare the `VolumeDiscount` schema in
...
- [ ] Task 15: Declare the `BlanketOrder` schema in
...
- [ ] Task 16: Declare the `CreditHold` schema in
...
- [ ] Task 17: Add `x-openregister-lifecycle` to `Quote` declaring all
...
- [ ] Task 18: Add `x-openregister-lifecycle` to `SalesOrder` declaring all
...
- [ ] Task 19: Add `x-openregister-lifecycle` to `Delivery` with status tracking
...
- [ ] Task 20: Add `x-openregister-lifecycle` to `Invoice` declaring all
...
- [ ] Task 21: Add `x-openregister-lifecycle` to `CreditNote` with status
...
- [ ] Task 22: Declare pricing-tier resolution as `x-openregister-aggregations`
...
- [ ] Task 23: Declare aged AR aggregation as `x-openregister-aggregations`
...
- [ ] Task 24: Declare sales-funnel KPI aggregations as
...
- [ ] Task 25: Implement BTW calculation per REQ-QOI-006 as lifecycle action
...
- [ ] Task 26: Implement Peppol e-invoicing per REQ-QOI-008 as lifecycle action
...
- [ ] Task 27: Implement credit-hold automation per REQ-QOI-010 as scheduled
...
- [ ] Task 28: Implement quote → order conversion (one-click action per
...
- [ ] Task 29: Implement delivery → invoice trigger per REQ-QOI-005
...
- [ ] Task 30: Implement order-confirmation advance invoicing per REQ-QOI-005
...
- [ ] Task 31: Implement consolidated monthly invoice per REQ-QOI-005 (if
...
- [ ] Task 32: Add 8 manifest navigation entries (Quotes, Sales Orders,
...
- [ ] Task 33: Confirm openconnector integration hooks for:
...
- [ ] Task 34: Confirm docudesk integration for quote and invoice PDF
  generation and storage (FK URI per bookkeeping-document-attachment-integration
  pattern)

- [ ] Task 35: Confirm bookkeeping-ifrs15-revenue integration: SalesOrder
...
- [ ] Task 36: Confirm bookkeeping-accounts-receivable-core integration: Invoice
  issued → AR open item created; AR ageing aggregation drives credit-hold logic
  (Task 27)

- [ ] Task 37: Confirm bookkeeping-general-ledger integration: Invoice issued →
...
- [ ] Task 38: Create seed data (3–5 realistic sales scenarios per administration)
...
- [ ] All 10 requirements (REQ-QOI-001 through REQ-QOI-010) have GIVEN/WHEN/THEN
  scenarios and are testable.
- [ ] All 11 schemas (Quote, QuoteLine, SalesOrder, SalesOrderLine, Delivery,
...
- [ ] All lifecycle transitions are declared with action names + calling patterns.
- [ ] All aggregations (pricing tier resolution, aged AR, sales-funnel KPIs) are
  defined with WHERE/GROUP BY clauses.
- [ ] Integration points (openconnector, docudesk, IFRS 15, AR, GL) are
  documented with FK references and action names.
- [ ] Risks and mitigations from proposal.md are addressed in design.md.
- [ ] Seed data covers quote → order → delivery → invoice happy path + edge cases
  (partial delivery, backorder, advance invoice, Peppol-dispatched invoice,
  credit-held customer).
- [ ] No PHP service classes authored (per ADR-031).
- [ ] All requirements reference external standards (Belastingdienst 13 fields,
