...
> out of scope here. The tasks below describe the work an `opsx-apply` cycle will execute
> against the `bookkeeping-ifrs-rj-dual-gaap` spec — they are recorded now so the
...
- [ ] Task 1: Confirm no `bookkeeping-ifrs-rj-dual-gaap` capability spec already exists,
...
- [ ] Task 2: Author `specs/bookkeeping-ifrs-rj-dual-gaap/spec.md` with
...
- [ ] Task 4: Author `design.md` with Reuse Analysis table, D1 (dual posting as single GL event
...
- [ ] Task 5: Declare the `AccountingFramework` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 6: Declare the `ChartOfAccountsMapping` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 7: Declare the `DualTransaction` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 8: Declare the `ReconciliationBridge` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 9: Declare the `StandardSpecificCalculation` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 10: Declare the `FrameworkElection` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 11: Extend T1 GL-materialisation logic (from `bookkeeping-general-ledger` implementation)
...
- [ ] Task 14: Implement `ReconciliationBridge` aggregation per REQ-DGAAP-004/005/006:
...
- [ ] Task 16: Implement FrameworkElection lifecycle and size-criteria warnings per REQ-DGAAP-010:
...
- [ ] Task 24: Add 4 manifest navigation entries (`Framework Configuration`, `Chart of Accounts Mapping`,
...
- [ ] Task 25: Update `openspec/architecture/adr-000-data-model.md` with
...
- [ ] Task 26: Extend `lib/Settings/shillinq_register.json` relations to link new schemas:
