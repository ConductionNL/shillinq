...
### REQ-AP-001: The system SHALL store vendor masters, AP invoices, and payment runs as OpenRegister-managed registers
...
### REQ-AP-003: The `APInvoice` schema SHALL declare a fixed minimum field set
...
### REQ-AP-004: The `APInvoice` schema SHALL declare an AP invoice lifecycle via `x-openregister-lifecycle`
...
### REQ-AP-005: AP approval routing SHALL be consumed from OR's approval-workflow abstraction
...
### REQ-AP-006: The `APInvoice` lifecycle SHALL implement conditional 3-way match

When the `received → matched` transition is triggered:
...
### REQ-AP-007: The `PaymentRun` schema SHALL declare SEPA pain.001 XML as an `x-openregister-calculations` field
...
### REQ-AP-008: The `VendorMaster` schema SHALL carry a declarative active/blocked/archived lifecycle
...
### REQ-AP-009: AP aging SHALL be declared as an `x-openregister-aggregations` query
