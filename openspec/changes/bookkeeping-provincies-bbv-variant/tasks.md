...
- [ ] Task 1: Confirm no existing `BBVComplianceDashboard` / `BudgetToProrammeLinker`
...
- [ ] Task 2: Confirm dependency on `add-shillinq-provincies-bbv-variant` is available
  and BBV variant enum includes `'provincie'` value; if not, file a blocker and defer
  remaining tasks

## Specification & Documentation

- [ ] Task 3: Author `specs/bookkeeping-provincies-bbv-variant/spec.md` with
...
- [ ] Task 4: Author `proposal.md` with High-Level Summary, Motivation, Affected
...
- [ ] Task 5: Author `design.md` with Goals, Non-Goals, 6 Design Decisions (D1: dashboard
...
- [ ] Task 6: Add 2 manifest pages to `src/manifest.json`:
...
- [ ] Task 7: Create `src/pages/BbvComplianceDashboard.vue` component:
...
- [ ] Task 8: Implement dashboard data queries in `stores/bbvDashboard.js` Pinia
  composable:
...
- [ ] Task 9: Implement dashboard chart rendering:
...
- [ ] Task 10: Implement Exceptions Alert section:
...
- [ ] Task 11: Add admin-configurable refresh interval to app settings:
...
- [ ] Task 12: Create `src/pages/BudgetToProrammeLinker.vue` (index + detail):
...
- [ ] Task 13: Implement filter bar for Linker index:
...
- [ ] Task 14: Implement bulk "Link to Programme" action:
...
- [ ] Task 15: Implement modal form validation:
...
- [ ] Task 16: Implement bulk save logic:
...
- [ ] Task 17: Implement detail page for GL line edit:
...
- [ ] Task 18: Verify Budget schema includes all required fields:
...
- [ ] Task 19: Verify GLLine schema includes all required fields:
...
- [ ] Task 20: Add Dutch (`l10n/nl.json`) + English (`l10n/en.json`) translation strings:
...
- [ ] Task 21: Create seed data file `lib/Settings/seeds/bbv-provincies-budgets-2026.json`:
...
- [ ] Task 22: For existing deployments with GL lines but no `programmaStructure`:
...
- [ ] Task 23: Verify adherence to company-wide ADRs:
...
- [ ] Task 24: PHPUnit tests (in `tests/Unit/`):
