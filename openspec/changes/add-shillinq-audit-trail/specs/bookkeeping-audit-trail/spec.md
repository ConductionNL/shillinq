...
### REQ-AT-001: Every bookkeeping register SHALL declare `x-openregister-audit: true` to enable OR's audit-trail-immutable abstraction
...
### REQ-AT-002: The OR audit trail SHALL record actor, action, timestamp, before/after diff, and hash chain for every state change
...
### REQ-AT-003: Shillinq SHALL expose a manifest entry into OR's audit-log UI pre-filtered to bookkeeping objects
...
### REQ-AT-004: Bookkeeping detail pages SHALL surface a per-object audit-trail side panel via the manifest
...
### REQ-AT-005: Audit-trail retention SHALL be governed by OR's archival + destruction workflow, not by a shillinq cleanup job
