...
- [ ] 1.1 `lib/Settings/connections.json`: fifteen keys in roster order,
...
- [ ] 2.1 `lib/Service/ConnectionReportService.php`: report the three called
  families by event, behind `class_exists`, never throwing.
- [ ] 2.2 `lib/BackgroundJob/ConnectionReportJob.php`, registered in
  `appinfo/info.xml`.
...
- [ ] 3.1 `src/manifest.d/external-adapters-w8.json`: `integriq/app_connection`
  index page, `requiresApp`, `showAdd: false`, Add integration header action;
  menu entry `query` and `visibleIf.appInstalled`.
- [ ] 3.2 `src/services/connectionFormatters.js` and
...
- [ ] 4.1 Remove `ExternalAdaptersAdminController`, its route,
  `ExternalAdaptersStatus.vue`, its registry entry and the tests that only
  served them.
- [ ] 4.2 `tests/e2e/external-connections.spec.ts` replaces the three roster
