...
- [x] Task 5: Declare the `XbrlInstance` schema in `lib/Settings/shillinq_register.json` with all REQ-SBR-002 fields (entryPoint, taxonomyVersion, financialStatementId, contextRefs, instanceDocumentUri, submissionReceiptUri, submittedAt, acceptedAt, rejectionReason, lifecycleState, administrationId)
...
- [x] Task 8: Extend the repair step under `lib/Repair/` to import NL-taxonomie mapping seeds idempotently (operator edits persist across re-runs) per REQ-SBR-006
