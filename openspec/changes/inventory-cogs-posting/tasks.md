...
- [ ] Task 1: Confirm no `inventory-cogs-posting` capability spec already exists,
...
- [ ] Task 2: Author `specs/inventory-cogs-posting/spec.md` with
...
- [ ] Task 4: Author `design.md` with Reuse Analysis table, D1 (lifecycle action on
...
- [ ] Task 5: Declare the `InventoryGLConfig` schema in
...
- [ ] Task 6: Extend the `InventoryValuation` schema in
...
- [ ] Task 7: Add `x-openregister-lifecycle` action `postCOGS` to `InventoryValuation`
...
- [ ] Task 8: Add `x-openregister-lifecycle` action `postReceipt` to `InventoryValuation`
...
- [ ] Task 9: Add `x-openregister-lifecycle` action `postVariance` to `InventoryValuation`
...
- [ ] Task 10: Add `"inventory"` to the `GLLine.subLedgerType` enum in
...
- [ ] Task 11: Seed `InventoryGLConfig` with default Dutch RGS account mappings for
...
- [ ] Task 12: Add 2 manifest navigation entries to `src/manifest.json` per REQ-CG-006:
...
- [ ] Task 13: Update `openspec/architecture/adr-000-data-model.md` with:
