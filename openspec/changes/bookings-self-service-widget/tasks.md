...
> **Implementation cycle (hydra-build).** The spec-only proposal has been implemented. Notes on ADR-driven deviations from the originally-recorded task text:
> - **ADR-037** — instead of a DB migration (Task 3), the `WidgetAccessKey` schema and the additive `isPublic`/`priceVisible`/`resourceId`/`timezone`/customer-capture extensions live in a `lib/Settings/register.d/30-bookings-self-service-widget.json` fragment (auto-merged by `SettingsService::deepMergeConfig`, union-safe). No monolith edit.
> - **Unification / ADR-022** — the widget reuses the canonical `Service`/`Resource`/`Appointment` schemas from `bookings-create-appointment` (NOT parallel `BookingService`/`BookingResource`). A customer is a Nextcloud contact entity: the public widget sets the canonical `customerId` to a deterministic guest id and stores captured name/email/phone as write-only fields (never echoed back, design D6). Real OR ObjectService API only (`findAll`/`saveObject`).
> - **ADR-016** — routes via `appinfo/routes.php` use the app-standard `/api/widget/{businessId}/...` prefix (the spec's `/ocs/v2.php/...` was the idealised form).
> - **Deferred** (need a live instance / external account / browser harness): Task 21 (Playwright E2E), Task 25 (`npm run build` artefact publish), Task 26 (3 partner-site verification), Task 28 (npm publish + 2FA), Task 29 (partner onboarding guide). Task 27 (PR) is handled by the hydra coordination layer, not opsx.
...
- [x] Task 5: Implement `src/Service/WidgetAuthService.php` with methods:
...
- [x] Task 6: Implement slot availability computation in `src/Service/SlotService.php`:
...
- [x] Task 7: Implement `src/components/SelfServiceWidget.vue` Vue component with:
...
- [x] Task 8: Implement `src/components/WidgetEmbed.js` (embed loader):
...
- [x] Task 18: Create admin UI for API key management in `src/views/WidgetSettings.vue`:
...
- [x] Task 23: Run `npm run lint` on `src/components/SelfServiceWidget.vue` and `widget/` to ensure code style passes (eslint, prettier)
- [x] Task 24: Run `composer test` to ensure all unit + integration tests pass (target: 100% coverage for widget-related code)
