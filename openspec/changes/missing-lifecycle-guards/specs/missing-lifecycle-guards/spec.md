...
method — every transition referencing one hard-fails (OpenRegister's
`LifecycleGuardRegistry::resolve()` throws `RuntimeException`, uncaught by
`LifecycleValidationListener`). Also fixes the previously-undiscovered
second half of the defect: a `requires` tag shaped `Class::method` can never
resolve via Nextcloud's container unless the app explicitly registers that
literal string as a DI service alias — true for every one of these 16 tags
...
- **Performance:** each guard performs at most a small, bounded number of `ObjectService::findAll()` lookups scoped by administration/period/vendor filters; no guard introduces an unbounded scan.
- **Accessibility:** N/A — no UI surface.
- **Internationalization:** deny messages are operator-facing English strings returned via `GuardResult::deny()`; no new translated UI strings are introduced (ADR-005 N/A — backend-only).

## Acceptance Criteria

- [ ] All 17 originally-missing classes + `PeriodCloseGuard::trialBalanceVerifies` resolve to a real implementation, or the declaration is removed with a documented reason.
- [ ] Every one of the 16 `Class::method`/bare-class tags this change fixes is registered in `Application.php` and provably resolves via the shared adapter.
- [ ] Every implemented guard has a passing bad-path (deny) and good-path (allow) unit test.
- [ ] The register.d/Application.php cross-reference regression test passes.
