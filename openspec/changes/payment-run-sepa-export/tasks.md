...
- [~] 7.3 Live-verify export — WIRING verified (route `POST /api/v1/payment-runs/{id}/export` resolves, controller executes, ADR-005 auth guard correctly 404s a synthetic run, PaymentRuns index renders, 0 console errors). End-to-end-with-data BLOCKED by the env's empty objects table (no Administration objects → `canAccess` 404s any run; documented OR seed-import gotcha). Export file-gen/lifecycle logic covered by passing unit tests (SepaPain001/Csv generators).
- [~] 7.4 Live-verify reconcile — WIRING verified (reconcile route + controller present). End-to-end-with-data BLOCKED by the same env seed gap. Match logic (EndToEndId primary + amount/IBAN fallback, full→reconciled, partial→mismatch note) covered by passing `PaymentRunReconciliationServiceTest` (5 tests) + the full/partial/fallback CAMT.053 fixtures.
