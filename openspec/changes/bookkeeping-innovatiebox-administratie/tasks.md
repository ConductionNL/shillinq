...
- [ ] **Task 1.1:** Scan codebase to confirm NO existing `QualifyingAsset`, `NexusCalculation`, `IBProfitAttribution`, `IBExpenseAllocation`, `CarryForwardLoss` schemas in `lib/Settings/shillinq_register.json`, `openspec/specs/**`, `openspec/changes/**`

- [ ] **Task 1.2:** Verify `bookkeeping-cost-centers-dimensions`, `bookkeeping-chart-of-accounts`, `bookkeeping-vpb-corporate-tax`, `bookkeeping-wbso-sno-administratie` dependency specs are published and available

- [ ] **Task 1.3:** Confirm OpenRegister audit-trail-immutable pattern (ADR-022) is available for immutable record enforcement

### Phase 2: Register Declarations (Data Model)

- [ ] **Task 2.1:** Declare `QualifyingAsset` schema in `lib/Settings/shillinq_register.json` with all fields per REQ-IBA-001; include `schema:CreativeWork` annotation for IP asset; default `status: 'valid'` on insert; validate S&O-verklaring format (S{jaar}/{6-cijfer}) on insert

- [ ] **Task 2.2:** Declare `NexusCalculation` schema per REQ-IBA-002; immutable after creation (enforce via audit-trail hook); uplift_factor hard-coded to 1.3; cap enforcement in calculation

- [ ] **Task 2.3:** Declare `IBProfitAttribution` schema per REQ-IBA-003; three-method enum; tariff hard-coded 0.09 (Wet Vpb art. 12b 2026); enforce unique constraint `(qualifying_asset_id, boekjaar)`

- [ ] **Task 2.4:** Declare `IBExpenseAllocation` schema per REQ-IBA-004; support kostensoort enum; include `exclusief_in_winstbepaling` flag for doorsnijdingsverbod; support S&O-uren reference via `bron_referentie`

- [ ] **Task 2.5:** Declare `CarryForwardLoss` schema per REQ-IBA-005; `verrekend_boekjaar` array with [{jaar, bedrag, saldo_na}]; status enum; immutable after creation

### Phase 3: Calculation & Aggregation Services (PHP)

- [ ] **Task 3.1:** Implement `NexusCalculationService` PHP class:
...
- [ ] **Task 3.2:** Implement `ProfitAttributionService` PHP class:
...
- [ ] **Task 3.3:** Implement `CarryForwardLossAggregation` PHP class:
...
- [ ] **Task 3.4:** Declare `innovatiebox-administratie` aggregation (x-openregister-aggregations):
...
- [ ] **Task 4.1:** Implement `DoorsnijdingsVerbodValidator` PHP class:
...
- [ ] **Task 4.2:** Implement `QualifyingAssetValidator` PHP class:
...
- [ ] **Task 6.1:** Register Vpb-aangifte innovatiebox-sectie docudesk template in `lib/Settings/docudesk-templates.json`:
...
- [ ] **Task 6.2:** Register innovatiebox-administratie summary template in `lib/Settings/docudesk-templates.json`:
...
- [ ] **Task 7.1:** Add Innovatiebox navigation to `src/manifest.json`:
...
- [ ] **Task 7.2:** Implement `CnIndexPage` + `CnDetailPage` for manifest entries (per ADR-024 Tier-4, no bespoke Vue):
...
- [ ] **Task 9.1:** Update `openspec/architecture/adr-000-data-model.md` with one-paragraph annotations for:
...
- [ ] **Task 10.1:** PHPUnit tests for `NexusCalculationService`:
...
- [ ] **Task 10.2:** PHPUnit tests for `ProfitAttributionService`:
