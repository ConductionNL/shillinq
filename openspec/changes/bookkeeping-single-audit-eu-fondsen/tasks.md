...
> `opsx-apply` cycle will execute against the `bookkeeping-single-audit-eu-fondsen`
...
- [ ] Task 1: Confirm no `bookkeeping-single-audit-eu-fondsen` capability spec already
...
- [ ] Task 2: Author `specs/bookkeeping-single-audit-eu-fondsen/spec.md` with
...
- [ ] Task 3: Author `proposal.md` referencing shared `nextcloud-app` spec and
...
- [ ] Task 4: Author `design.md` with Reuse Analysis table, D1 (seven registers:
...
- [ ] Task 5: Declare `eu-project` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 6: Declare `eligibility-rule` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 7: Declare `segregated-ledger` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 8: Declare `eu-expenditure` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 9: Declare `supporting-document` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 10: Declare `irregularity-report` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 11: Declare `audit-trail` schema in `lib/Settings/shillinq_register.json`
...
- [ ] Task 12: Implement segregated-ledger reconciliation logic per REQ-EUF-002 —
...
- [ ] Task 13: Implement cost-eligibility validation per REQ-EUF-001 + REQ-EUF-011 —
...
- [ ] Task 14: Implement VAT-treatment auto-flagging per REQ-EUF-002 + REQ-EUF-011 —
...
- [ ] Task 15: Implement bewijsstukken validation per REQ-EUF-004 — on declaration_period
...
- [ ] Task 16: Implement aanbestedings-threshold detection per REQ-EUF-005 — on
...
- [ ] Task 18: Implement irregularity-reporting logic per REQ-EUF-007 — on
...
- [ ] Task 19: Implement financial-correction booking per REQ-EUF-008 — on
...
- [ ] Task 21: Implement audit-portaal per REQ-EUF-006 + REQ-EUF-009 — read-only
...
- [ ] Task 22: Implement audit-trail immutability per REQ-EUF-009 — AuditTrail
...
- [ ] Task 23: Implement visibility-tracking per REQ-EUF-010 — optional SupportingDocument
...
- [ ] Task 24: Seed `eligibility-rule` register with production rules per EU-fondsen
...
- [ ] Task 25: Integrate with docudesk per ADR-022 — SupportingDocument.source_uri
...
- [ ] Task 28: Add manifest navigation entries for Shillinq — "EU-projecten" (list
...
- [ ] Task 29: Add tests for segregated-ledger reconciliation (Task 12), cost-eligibility
...
- [ ] Task 30: Author release notes documenting new spec, new registers, new manifest
