...
### Requirement: REQ-MC-001: Multi-currency bank account management SHALL be declared as `BankAccount.primaryCurrency` extension + `CurrencyBalance` register
...
### Requirement: REQ-MC-002: The `BankAccount` schema extension SHALL declare `primaryCurrency` and support multi-currency operations
...
### Requirement: REQ-MC-003: The `CurrencyBalance` schema SHALL track per-currency balance snapshots per account
...
### Requirement: REQ-MC-004: Per-currency balance queries SHALL filter and aggregate without custom PHP
...
### Requirement: REQ-MC-005: Manifest navigation SHALL expose Currency Balances index + detail pages
