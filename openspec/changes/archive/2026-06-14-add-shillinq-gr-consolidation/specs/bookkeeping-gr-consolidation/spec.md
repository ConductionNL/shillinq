...
### Requirement: REQ-GRC-001: The system SHALL declare a `GRDeelnemer` register identifying each deelnemer of a gemeenschappelijke regeling
...
### Requirement: REQ-GRC-002: The system SHALL declare a `GRVerdeelsleutel` register parameterising per-cost-cluster apportionment
...
### Requirement: REQ-GRC-003: `GLLine` SHALL carry an `eliminationFlag` so inter-GR transactions can be excluded from consolidated views
...
### Requirement: REQ-GRC-004: The system SHALL produce a separate GR jaarrekening AND per-deelnemer doorbelasting via declarative aggregation
...
### Requirement: REQ-GRC-005: A doorbelasting SHALL materialise as a balanced `GLTransaction` in the deelnemer's administration when the deelnemer also runs shillinq
...
### Requirement: REQ-GRC-006: GR sector view SHALL be reachable through a feature-flag-controlled manifest navigation entry
