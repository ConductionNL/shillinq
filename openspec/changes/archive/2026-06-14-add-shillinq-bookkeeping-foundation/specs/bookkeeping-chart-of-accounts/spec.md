...
- **WHEN** an object `{accountNumber: "1000", name: "Kas", accountType: "assets", currency: "EUR", administrationId: "adm-1", lifecycleState: "active"}` is validated
...
- **GIVEN** parent account `1000 Kas` and child account `1010 Kas winkel`
...
- **GIVEN** account `4100 Omzet` in state `active`
...
- **GIVEN** account `4100 Omzet` has open postings with a non-zero
  balance
...
- **GIVEN** a competent Dutch SMB bookkeeper persona reads
  `rgs-3.5-mkb.json`
- **THEN** the structure (activa / passiva / eigen vermogen / omzet /
  kosten top-level groupings) SHALL match the RGS 3.5 SMB reference
...
- **GIVEN** the template is seeded and the operator has renamed
  account `4100` from `Omzet` to `Omzet binnenland`
...
Grootboekschema` or top-level — exact placement settled in the
