...
### Requirement: REQ-AP-002 — The `Payee` schema SHALL declare a fixed minimum field set

The `Payee` schema MUST declare the following fields with the listed
types and required flags. Schema validation MUST reject Payee records
...
### Requirement: REQ-AP-003 — The `APTransaction` schema SHALL declare a fixed minimum field set with line breakdown

The `APTransaction` schema MUST declare the following fields with the
listed types and required flags. Schema validation MUST reject
APTransaction records missing any required field or whose `lines`
array is empty.
...
### Requirement: REQ-AP-004 — `APTransaction` SHALL declare a declarative draft → issued → paid lifecycle with dunning-driven overdue branch
...
### Requirement: REQ-AP-005 — AP dunning workflow SHALL consume OR's dunning-workflow abstraction; no app-local dunning service
...
### Requirement: REQ-AP-006 — Aged Payables Detail report SHALL show per-invoice breakdown grouped by vendor and due-date bucket
...
### Requirement: REQ-AP-007 — Aged Payables Summary report SHALL show vendor totals grouped by aging bucket
...
### Requirement: REQ-AP-008 — Aged Payables Timeline report SHALL show payment due dates with amounts and vendor summary
...
### Requirement: REQ-AP-009 — Payment matching SHALL transition AP invoices from issued → paid via bank-reconciliation confirmation

Payment matching MUST transition AP invoices from `issued` to `paid` when an
...
than `totalAmount`, the invoice SHALL transition to `partially-paid` instead;
...
### Requirement: REQ-AP-010 — Manifest navigation SHALL include four entries for AP core workflow
...
### Requirement: REQ-AP-011 — Seed data SHALL include 3 vendors and 5–8 AP invoices with realistic Dutch details
