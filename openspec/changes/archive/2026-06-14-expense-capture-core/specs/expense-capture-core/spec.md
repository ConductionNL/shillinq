...
### Requirement: REQ-EC-001: Expense capture SHALL be declared as `Receipt`, `MileageEntry`, `PerDiem`, and `ExpenseClaimEntry` registers
...
### Requirement: REQ-EC-002: The `Receipt` schema SHALL capture photo, amount, date, currency, and category
...
### Requirement: REQ-EC-003: The `MileageEntry` schema SHALL capture distance, rate, and auto-calculated total
...
### Requirement: REQ-EC-004: The `PerDiem` schema SHALL capture country, nights, and auto-looked-up daily rate
...
### Requirement: REQ-EC-005: The `ExpenseClaimEntry` schema SHALL group N receipts, mileage, and per-diem into one claim
...
### Requirement: REQ-EC-006: `ExpenseClaimEntry` approval SHALL consume OR's approval-workflow; no app-local approval table
...
### Requirement: REQ-EC-007: `ExpenseClaimEntry` SHALL declare a declarative draft → submitted → approved → posted → reimbursed lifecycle
...
### Requirement: REQ-EC-008: Photo upload SHALL accept JPEG, PNG, and PDF file types with size limit
...
### Requirement: REQ-EC-009: Multi-currency conversion SHALL use rates from the multi-currency capability; stored in base EUR
...
### Requirement: REQ-EC-010: Expense claim approval workflow SHALL route escalations per threshold policy
...
### Requirement: REQ-EC-011: Expense report aggregation SHALL group by category, cost centre, and date range
...
### Requirement: REQ-EC-012: Expense claim materialisation SHALL post a balanced GL entry with cost-centre allocation per T1 pattern
...
### Requirement: REQ-EC-013: Manifest navigation SHALL include Receipts, Expense Claims, and Mileage Log
