# Spec: shillinq-bank-statement-wizard

**Status:** proposed
**Scope:** shillinq
**Depends on:** add-shillinq-bank-reconciliation, shillinq-financial-dashboard-actions

A guided first-import wizard launched from the Financial overview dashboard,
plus the server-side import endpoint that parses a CAMT.053 / MT940 / CSV file
into a `BankStatement` and its `BankStatementLine` records.
