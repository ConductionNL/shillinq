...
### Requirement: REQ-SISA-001: Single Information Single Audit SHALL be declared as `SisaReport` + `AuditDocument` + `ComplianceAuditTrail` registers, not a parallel audit database
...
### Requirement: REQ-SISA-002: The `SisaReport` schema SHALL declare a fixed minimum field set
...
### Requirement: REQ-SISA-003: The `AuditDocument` schema SHALL declare a fixed minimum field set
...
### Requirement: REQ-SISA-004: `AuditDocument` SHALL declare a declarative draft → issued → signed lifecycle
...
### Requirement: REQ-SISA-005: The `ComplianceAuditTrail` schema SHALL declare a fixed minimum field set
...
### Requirement: REQ-SISA-006: `ManagementLetter` schema holds auditor communication
...
### Requirement: REQ-SISA-007: `SisaReport` aggregation SHALL compute on-time settlement compliance
...
### Requirement: REQ-SISA-008: `SisaReport` aggregation SHALL group findings by severity
...
### Requirement: REQ-SISA-009: Audit opinion SHALL be assigned via declarative rule or guarded service
...
### Requirement: REQ-SISA-010: Grant records MAY carry `isSISAEligible` flag for SiSa filtering
...
### Requirement: REQ-SISA-011: `SisaReport` manifest entries SHALL expose Compliance Audit, Management Letter, and SiSa Reports
...
### Requirement: REQ-SISA-M001: Audit trail is captured automatically on state transitions
