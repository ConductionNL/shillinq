...
### REQ-BCF-001: The system SHALL administer Btw-compensatiefonds claims as an OpenRegister-managed `BcfClaim` register
...
### REQ-BCF-002: The `BcfClaim` schema SHALL declare a fixed minimum field set
...
### REQ-BCF-003: BCF compensable accounts SHALL be flagged via `BbvAccountMapping.bcfCompensable`
...
### REQ-BCF-004: Claim aggregation SHALL be a declarative aggregation over GL postings filtered by the compensable flag
...
### REQ-BCF-005: Partial-compensability accounts SHALL be supported via a `compensablePercentage` field
...
### REQ-BCF-006: The `BcfClaim` lifecycle SHALL be declarative per ADR-031
...
### REQ-BCF-007: Submission to the BCF SHALL be an OR `ScheduledWorkflow` consuming the `digikoppeling-bcf` OpenConnector source
...
### REQ-BCF-008: BCF claims SHALL be reachable through the shillinq manifest navigation
...
### REQ-BCF-009: Audit trail and retention SHALL be consumed from OR's abstractions
