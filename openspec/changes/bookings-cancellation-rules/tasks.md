...
- [~] Task 26: PR creation — handled by the Hydra build flow (not an opsx task). **Handoff**: prior cycle landed via PR #279; this finish-2 cycle merges locally via `--no-ff` per build brief (no Codeberg push). Closes as [~].
