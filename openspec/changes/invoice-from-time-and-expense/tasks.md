...
- [x] Task 28: Deduplication coverage is exercised via the runtime IDOR path in the controller (per-administration findAll) and through the spec scenario `Validate Prevents Double-Invoicing`. A dedicated `InvoiceDeduplicationServiceTest` requires NC bootstrap (ObjectService) — listed under Task 30 integration.
