...
### REQ-SUB-002: Subsidie required fields
...
### REQ-SUB-003: Subsidie lifecycle
...
### REQ-SUB-005: Disbursement GL posting
...
### REQ-SUB-007: RepaymentInstallment register
...
### REQ-SUB-009: Settlement plan sub-state

When `uitbetaald → teruggevorderd`, the operator MAY create a settlement plan (`RepaymentInstallment` records) if immediate repayment is not viable. The Subsidie's `state` remains `teruggevorderd` with sub-state indication of payment plan existence.

### REQ-SUB-010: Compliance citation
