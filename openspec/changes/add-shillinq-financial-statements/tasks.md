...
- [ ] Task 1: Confirm no `bookkeeping-financial-statements` capability spec already exists and that no `lib/Service/BalanceSheet*` / `lib/Service/ProfitAndLoss*` / `lib/Service/CashFlow*` / `lib/Service/Statement*` / `lib/Service/Xbrl*` / `lib/Service/Sbr*` PHP classes are present (per ADR-031 anti-pattern enumeration)
