...
- [ ] Task 1: Confirm no `bookkeeping-programmabegroting` capability spec already exists, no 
...
- [ ] Task 2: Confirm `bookkeeping-bbv-compliance` is available (or at least the `BBVTaakveldCatalogus` 
  table is present) and confirm `bookkeeping-budget-forecast` exists for forecast cijfers integration

- [ ] Task 3: Confirm OR's `ScheduledWorkflow` is available and stable for scheduled sluitend-criteria 
...
- [ ] Task 4: Confirm CBS iv3-koppelvlak API is documented and determine whether an OpenConnector 
  source for iv3-aanlevering will be available in this cycle

- [ ] Task 5: Author `specs/bookkeeping-programmabegroting/spec.md` with `Status: proposed` / 
...
- [ ] Task 6: Author `proposal.md` referencing the shared `nextcloud-app` spec and including 
...
- [ ] Task 7: Author `design.md` with Reuse Analysis table, D1 (canonical Taakveld view, Programma 
...
- [ ] Task 8: Declare the `Programmabegroting` schema in `lib/Settings/shillinq_register.json` with 
...
- [ ] Task 9: Declare the `Programma` schema in `lib/Settings/shillinq_register.json` with all 
...
- [ ] Task 10: Declare the `Taakveld` schema in `lib/Settings/shillinq_register.json` with all 
...
- [ ] Task 11: Declare the `Indicator` schema in `lib/Settings/shillinq_register.json` with all 
  REQ-004 fields (programmaId, code, omschrijving, eenheid, nulwaarde, streefwaarde, realisatie, 
  bron, administrationId)

- [ ] Task 12: Declare the `Investering` schema in `lib/Settings/shillinq_register.json` with all 
...
- [ ] Task 13: Declare the `Reserve` schema in `lib/Settings/shillinq_register.json` with all 
  REQ-006 fields (begrotingId, type, naam, beginsaldo, toevoegingen, onttrekkingen, eindsaldo, 
  bestemmingsdoel, administrationId)

- [ ] Task 14: Declare the `Voorziening` schema in `lib/Settings/shillinq_register.json` with all 
  REQ-006 fields (begrotingId, naam, grondslag, beginsaldo, dotaties, vrijval, aanwendingen, 
  eindsaldo, administrationId)

- [ ] Task 15: Declare the `Paragraaf` schema in `lib/Settings/shillinq_register.json` with all 
...
- [ ] Task 16: Declare the `Meerjarenraming` schema in `lib/Settings/shillinq_register.json` with 
...
- [ ] Task 17: Declare the `Begrotingswijziging` schema in `lib/Settings/shillinq_register.json` 
...
- [ ] Task 18: Add `x-openregister-lifecycle` to `Programmabegroting` declaring transitions in 
...
- [ ] Task 19: Implement the sluitend-evaluation lifecycle action per REQ-008 + REQ-011 — query 
...
- [ ] Task 20: Implement toezichtRegime determination per D5 and REQ-011 — combine sluitend-flags 
  with prior 4-year resultaat via aggregation; emit event if regime shifts from repressief to 
  preventief

- [ ] Task 21: Implement Paragraaf auto-creation on Programmabegroting draft per REQ-007 — when 
  Programmabegroting.status → draft, create 7 Paragraaf records (one per type) with empty 
  narrative and placeholder kerncijfers

- [ ] Task 22: Implement Meerjarenraming seeding on Programmabegroting draft per REQ-008 — when 
...
- [ ] Task 23: Declare aggregations per REQ-003 + REQ-010 — Taakveld validation (code against 
...
- [ ] Task 24: Declare Programma aggregations per REQ-002 — automatic computation of batenTotaal, 
  lastenTotaal, saldoVoorMutaties, saldoNaMutaties from child Taakvelde (no rounding drift)

- [ ] Task 25: Declare Investering.kapitaallastenSchedule computation per REQ-005 — depreciation 
  schedule from afschrijvingstermijn and eersteAfschrijvingsjaar

- [ ] Task 26: Implement Begrotingswijziging materialization per REQ-009 — when wijziging.status → 
...
- [ ] Task 27: Add GL posting precondition per REQ-010 — when `JournalEntry` is materialized 
...
- [ ] Task 28: Implement iv3-export per REQ-012 — aggregate baten/lasten per taakveld from 
...
- [ ] Task 29: Implement EMU-saldo export per REQ-012 — compute saldo per Wet Hof / SNA-2010 
...
- [ ] Task 30: Implement JSON export per REQ-012 — serialize vastgestelde Programmabegroting with 
...
- [ ] Task 31: Add manifest navigation entries per `design.md` / REQ-012 — 3+ entries 
...
- [ ] Task 32: Update `openspec/architecture/adr-000-data-model.md` with 
  `Programmabegroting`/`Programma`/`Taakveld`/`Investering`/`Reserve`/`Voorziening`/`Paragraaf`/
...
- [ ] Task 33: Implement configuration form or UI for entering nominale-ontwikkeling annually per 
  D7 — default to 2% per IPO; require entry before Programmabegroting.status → in-behandeling

- [ ] Task 34: Wire up ScheduledWorkflow (or fallback T2 background task per opsx-ff) for 
