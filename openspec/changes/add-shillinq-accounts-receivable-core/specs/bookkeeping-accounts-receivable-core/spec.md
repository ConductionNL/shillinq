...
### REQ-AR-001: Accounts receivable SHALL be declared as `CustomerMaster` + `ARInvoice` + `DunningRecord` registers, not duplicates of GL
...
### REQ-AR-003: The `ARInvoice` schema SHALL declare a fixed minimum field set with line breakdown
...
### REQ-AR-004: `ARInvoice` SHALL declare a declarative draft → issued → paid lifecycle with dunning-driven overdue branch
...
### REQ-AR-005: AR dunning workflow SHALL consume OR's dunning-workflow abstraction; no app-local dunning service
...
### REQ-AR-006: Credit-limit checks SHALL be a declarative aggregation surfaced on AR draft creation
...
### REQ-AR-007: Payment matching SHALL transition `issued`/`partially-paid` → `paid` via bank-reconciliation events; no shillinq matcher service
...
### REQ-AR-008: AR aging SHALL be declared as an `x-openregister-aggregations` query, not a PHP report builder
...
### REQ-AR-009: AR invoice issuance SHALL declare the field shape T4 needs for UBL 2.1 / Peppol BIS 3.0 outbound, even though dispatch ships in T4
