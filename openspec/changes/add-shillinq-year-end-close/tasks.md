...
- [x] Task 7: Implement closing actions as lifecycle action handlers — `open → closing` emits the retained-earnings transfer T1 `JournalEntry` (manual sub-type) per REQ-YEC-004; `closing → closed` emits the next-year opening-balance T1 `JournalEntry` (balance-sheet accounts only) per REQ-YEC-005
...
- [x] Task 10: Add Fiscal Years navigation + pages to `src/manifest.json` (menu entry `Bookkeeping > Fiscal Years`, `type: index` page binding to `FiscalYear`, `type: detail` page surfacing close + reopen actions gated by role per REQ-YEC-006); `node tests/validate-manifest.js` exits 0
