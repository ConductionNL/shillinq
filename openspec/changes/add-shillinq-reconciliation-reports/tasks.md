...
- [ ] Task 3: Author `proposal.md` referencing the shared `nextcloud-app` spec and including Affected Projects / Scope / Risks (engine support, report-engine creep, launchpad dep drift) / Rollback / Open Questions per shillinq config.yaml `rules.proposal`
- [ ] Task 4: Author `design.md` with Decisions (saved-query not report engine, severity as calculated field, launchpad runtime-GraphQL no install-time dep, cross-administration engine-vs-guard resolved in opsx-ff) and Reuse Analysis table per hydra `rules.design`
...
- [ ] Task 12: Update `openspec/architecture/adr-000-data-model.md` with a one-paragraph reconciliation note introducing `Budget` and noting the four saved-query records consumed by launchpad via runtime GraphQL (no install-time dep per `feedback_launchpad-no-or-dependency.md`)
