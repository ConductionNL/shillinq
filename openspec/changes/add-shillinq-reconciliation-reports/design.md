...
saved queries against OR registers; mydash discovers them through the
...
### D3 — mydash consumes via runtime GraphQL, no install-time dep

Per `feedback_mydash-no-or-dependency.md`, mydash MUST NOT list
shillinq as a dependency. Saved queries declared in shillinq's
register are exposed via OR's GraphQL schema; mydash widget code
queries that schema at runtime and renders the results. Neither app
imports the other; the contract is the GraphQL schema.

**Alternative considered**: mydash imports shillinq as a dependency
and reads register objects directly. Rejected — that couples the BI
surface to the bookkeeping app's install lifecycle and makes mydash
