...
### REQ-REC-001: Bank reconciliation SHALL be declared as `BankReconciliation` + `ReconciliationMatch` + `ReconciliationReport` registers
...
### REQ-REC-003: `BankReconciliation` SHALL declare the reconciliation workflow lifecycle
...
### REQ-REC-005: `ReconciliationMatch` SHALL record automatic and manual matches
...
### REQ-REC-007: Variance reporting SHALL be declared as `x-openregister-aggregations` queries
...
### REQ-REC-008: Bank reconciliation SHALL be reachable through the shillinq manifest navigation
...
### REQ-REC-009: Bank reconciliation lifecycle transitions SHALL be audit-trailed with signed-off timestamp
