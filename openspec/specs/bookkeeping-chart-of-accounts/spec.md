...
@e2e exclude REST API contract via OpenRegister: covered by Newman/PHPUnit — no Shillinq-specific DOM to assert
...
@e2e exclude static code-review check: verified by PHPUnit/linter (absence of Mapper classes) — not browser-observable
...
@e2e exclude OR schema validation: tested via OR's schema-validation API or PHPUnit — no browser DOM to assert
...
@e2e exclude OR relation engine: tested via OR's relation-traversal API — no Shillinq-specific DOM
...
@e2e exclude OR relation validator: tested via OR's API or PHPUnit — no browser DOM to assert
...
@e2e exclude MCP/API contract: tested via OR's MCP discovery endpoint — no Shillinq browser DOM to assert
...
@e2e exclude OR lifecycle precondition: state-machine guard enforced by OR on object save — no Shillinq browser UI to assert
...
@e2e exclude OR lifecycle precondition: balance guard enforced by OR on transition — no Shillinq browser UI to assert
...
@e2e exclude static JSON validation: tested by `validate-manifest.js` or PHPUnit — no browser DOM to assert
...
@e2e exclude domain-expert content review: manual or static diff check — not browser-automatable
...
@e2e exclude PHP repair step: tested via PHPUnit InitializeSettingsTest — not browser-observable
...
@e2e exclude PHP repair step idempotency: tested via PHPUnit — not browser-observable
...
@e2e exclude unbuilt UI: ChartOfAccounts page not present in v0.1.0 manifest (Dashboard + Settings only) — pending ChartOfAccounts manifest entry and build
...
@e2e exclude unbuilt data dependency: requires seeded Account records (repair step not yet running in test env) — pending seed fix tracked in issue
...
@e2e exclude OR uniqueness constraint: enforced at data-layer via OR's single-true field validator — no Shillinq browser UI to assert
...
@e2e exclude OR constraint relaxation sequence: data-layer logic tested via OR's API or PHPUnit — no browser DOM to assert
