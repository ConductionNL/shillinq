...
## As-built requirements promoted from the archived change

The `bookkeeping-trial-balance` change was archived without its delta spec
being promoted into this file. Its requirements survived only in
`openspec/changes/archive/2026-06-14-bookkeeping-trial-balance/specs.md`,
which is a per-change document with no canonical home, so the shipped
service, controller and tests had nothing canonical to trace to. The
requirements below are promoted from that document unchanged in substance.
Their numbering continues past REQ-TB-006 and does not collide with it.

**READ THE NOTES AT THE END OF THIS FILE BEFORE TREATING THIS SECTION AS
SETTLED.** REQ-TB-001 above forbids the very classes REQ-TB-008 and
REQ-TB-018 below require. That contradiction is recorded, not resolved.

### REQ-TB-008: Backend trial-balance computation service

`TrialBalanceService::compute(string $administrationId, string $periodId,
array $filters = []): array` MUST return trial-balance data for one
administration and period: one row per `(periodId, accountNumber)` carrying
account name and type, opening balance, debit and credit movement, closing
balance, currency and parent account number, sorted by account number, with
the totals per account type alongside.

#### Scenario: The service aggregates a period by account

- **GIVEN** `TrialBalanceService::compute('admin-2026', '2026-Q1')`
- **WHEN** the service reads the period's GL activity and groups it by
  account number
- **THEN** it returns rows sorted by account number, each with correctly
  summed debit and credit movement and a closing balance of opening +
  (debit − credit)

### REQ-TB-009: Trial-balance API endpoint

`GET /index.php/apps/shillinq/api/trial-balance?period_id=<period>&administration_id=<admin>`
MUST return the computed trial balance as JSON: the row array, the row
count, and a totals object carrying total debit, total credit and the
per-account-type subtotals.

#### Scenario: A valid request returns rows and totals

- **GIVEN** a GET request naming an existing period and administration
- **WHEN** the controller serves it
- **THEN** the response is HTTP 200 carrying the rows, the count and the
  totals object

### REQ-TB-014: Trial-balance query performance

For a typical administration — on the order of 10 000 accounts and 100 000
GL lines in the period — the trial-balance query MUST complete in under two
seconds.

#### Scenario: A large period stays within budget

- **GIVEN** an administration with 10 000 accounts and 100 000 postings in
  the period
- **WHEN** the trial balance is computed
- **THEN** the computation completes in under two seconds

### REQ-TB-015: A missing or invalid period is refused, not guessed

When `period_id` is absent or is not a valid period identifier, the API
MUST answer HTTP 400 with a message naming the parameter. It MUST NOT
default to a period, and MUST NOT return an empty result as though the
period existed and was empty.

#### Scenario: The period parameter is missing

- **GIVEN** a request with no `period_id`
- **WHEN** the controller validates it
- **THEN** the response is HTTP 400 with a message naming `period_id`

### REQ-TB-016: Reading a trial balance requires read access to the administration

A caller without read access to the named administration MUST NOT receive
its trial balance. The check MUST be made against the caller's own
membership of that administration, never against a value the caller
supplied about themselves.

#### Scenario: A caller outside the administration is refused

- **GIVEN** a user with no read access to `admin-2026`
- **WHEN** they request the trial balance for `admin-2026`
- **THEN** the request is refused and no row of that administration's data
  is returned

### REQ-TB-017: Multi-tenancy isolation

Every read on the trial-balance path MUST be scoped to the requested
administration. Two administrations MUST NOT be able to observe each
other's rows or totals through any parameter combination.

#### Scenario: Two administrations cannot see each other

- **GIVEN** administrations `org-A` and `org-B`, each with posted GL
- **WHEN** a member of `org-A` computes the trial balance
- **THEN** the result contains rows of `org-A` only, and no total is
  inflated by `org-B` activity

### REQ-TB-018: PHP fallback when the aggregation engine cannot serve the query

When OpenRegister's aggregation engine is unavailable, or cannot express the
cross-schema join the trial balance needs, `TrialBalanceService::compute()`
MUST fall back to reading the `GLLine` rows and the `Account` hierarchy and
summing them in PHP, returning the same row shape as the aggregation path.

#### Scenario: The aggregation engine is unavailable

- **GIVEN** OR's aggregation surface is unavailable
- **WHEN** the trial balance is computed
- **THEN** the service reads GL lines filtered by period, reads the account
  hierarchy, aggregates in memory, and returns the same shape it would have
  returned from the aggregation engine
...
a reviewer to confirm that no `lib/Service/TrialBalance*.php` file exists.
`lib/Service/TrialBalanceService.php` and `lib/Service/TrialBalanceCalculator.php`
both exist and have since the change shipped. REQ-TB-008 and REQ-TB-018,
promoted above from that same change's own delta spec, require them.

The two requirements cannot both hold. This file records both because
deleting either one would misrepresent the repository: the code follows
REQ-TB-008/018, and REQ-TB-001 is the declarative-first design decision
(ADR-031) that was never reconciled with it. A human owns this call —
either the services become thin renderers over a declarative aggregation
and REQ-TB-008/018 are narrowed accordingly, or REQ-TB-001 is amended to
permit them. Nothing in this file should be read as having made that
decision.
