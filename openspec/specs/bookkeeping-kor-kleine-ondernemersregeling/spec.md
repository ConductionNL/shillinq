...
This specification defines the requirements for bookkeeping kor kleine ondernemersregeling in the Shillinq Nextcloud accounting application, establishing the data model, behaviour and acceptance scenarios for this capability.

## Requirements

@e2e exclude unbuilt UI: KOR page not yet implemented


### REQ-KOR-001: The system SHALL administer Kleine-ondernemersregeling status as an OpenRegister-managed `KorRegime` register
...
### REQ-KOR-003: KOR thresholds SHALL be seeded, not hard-coded
...
### REQ-KOR-004: YTD revenue SHALL be derived via declarative aggregation, not a service method
...
### REQ-KOR-005: The `KorRegime` lifecycle SHALL automatically transition on threshold crossings
...
### REQ-KOR-008: A KOR-status widget SHALL be declared via `x-openregister-widgets`
...
### REQ-KOR-009: KOR administration SHALL be reachable through the shillinq manifest navigation
...
### REQ-KOR-010: Audit trail and retention SHALL be consumed from OR's abstractions
