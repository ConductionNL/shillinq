...
# payment-run-four-eyes Specification

## Purpose

Enforces server-side segregation of duties (four-eyes control) on the outgoing SEPA
`PaymentRun.approve` lifecycle transition — the hard control point before an approved batch can be
exported to a bank file and money leaves the administration. The user who APPROVES a payment run
MUST NOT be the user who PREPARED it. Preparer identity is derived exclusively from OpenRegister's
immutable audit trail (ADR-022), never a hand-rolled parallel actor log, and the control fails
closed whenever it cannot positively establish that the approver differs from the preparer.
...
The `PaymentRun.approve` lifecycle transition (`draft → approved`) SHALL be gated by a server-side
segregation-of-duties guard (`FourEyesPaymentRunGuard`, wired via the schema's
`transitions.approve.requires` DI tag). The guard SHALL DENY the transition when the approving user
(the authenticated caller) is the same user who prepared the batch. The preparer identity SHALL be
derived exclusively from OpenRegister's immutable audit trail (ADR-022) — the `create` actor plus
any `update` actor recorded while the batch was a draft — and SHALL NOT be stored in a bespoke
`preparedBy`/`approvedBy` field that could drift from the audit trail. The control SHALL run on the
object-save path so a direct save that changes `lifecycleState` is gated identically to the
`/transition` endpoint.
