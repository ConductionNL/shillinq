...
- **GIVEN** the schema
- **WHEN** an object `{journalNumber:"M-2026-0001", entryDate:"2026-03-01", description:"Bankkosten", lines:[{accountNumber:"4500", side:"debit", amount:25}, {accountNumber:"1000", side:"credit", amount:25}], journalType:"manual", approvalState:"not-required", administrationId:"adm-1", state:"draft"}` is saved
...
- `recurring` — template that materialises postings on a cadence
  (abonnementen, maandelijkse afschrijvingen, periodieke transitorische posten)
- `reversing` — entry whose materialised posting is automatically
  inverted at the start of a designated future period (periode-eind
  accruals, vooruitbetaalde kosten)
...
- **GIVEN** a `reversing` journal posted in `periodId: "2026-12"`
  with `reversesOn: "2027-01"`, debit `4500 Abonnementkosten` €1 000,
  credit `2100 Overlopende passiva` €1 000
- **WHEN** period `2027-01` begins
- **THEN** a new `GLTransaction` MUST be materialised with debit
  `2100 Overlopende passiva` €1 000, credit `4500 Abonnementkosten`
...
- **GIVEN** a docudesk attachment exists at URI
  `docudesk://attachments/0d4e9c7f-…/factuur-2026-0001.pdf`
...
- **THEN** no `GLTransaction` MUST be created; **AND** the journal
  ...
  "boeking is niet gebalanceerd" error surfaced from REQ-GL-005.
...
Journaalposten`) with a `type: index` page binding to the `JournalEntry`
...
- **THEN** the transition MUST fail with a "storneer eerst de
  grootboektransactie" error.
...
- **THEN** `J1` MUST NOT appear; **AND WHEN** the
  "inclusief geannuleerde" filter is applied, `J1` MUST appear with its
