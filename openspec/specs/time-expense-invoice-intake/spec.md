...
# Spec: Time & Expense Invoice Intake

**Status:** done
**Scope:** shillinq
**OpenSpec changes:**
- [time-expense-invoice-intake](../../changes/archive/2026-07-12-time-expense-invoice-intake/) _(archived 2026-07-12)_
...
- [x] `POST /apps/shillinq/api/billing/time-intake` is registered in
  `appinfo/routes.php` with an auth attribute (no `#[PublicPage]`).
- [x] Anonymous request returns `401`; client `administrationId` is ignored.
- [x] A valid T&M batch drafts exactly one `BillableInvoice` (`status=draft`)
  with one line per entry.
- [x] Each entry materialises a `UrenRegistratie` row with `externalId`,
  `sourceApp`, `sourceBatchId`.
- [x] Replaying the same `batchId` returns `duplicated: true` and no
  duplicate objects.
- [x] Reused `batchId` with a different payload returns `409`.
- [x] A cross-batch duplicate `externalId` returns `422`.
- [x] Non-T&M `billingModel` returns `422`; empty/malformed batch returns `400`.
- [x] Response matches `{invoiceId, invoiceNumber?, status:"draft", lines:n, duplicated:bool}`.
- [x] `TimeIntakeBatch` schema and the three `UrenRegistratie` provenance
  fields ship as a `register.d` fragment (canonical register untouched).
- [x] Seed data exists for `TimeIntakeBatch` and the provenance fields.

## Notes

- Cross-repo consumer: pipelinq change `time-billing-handoff-emit` codes
  against the frozen contract in the archived change's `contract.md`. This
  change shipped first; pipelinq's manifest capability moves from
  `blocked-on-prereq` to buildable now that the route is live.
- `rateRef` resolution and the batch-wide `rateCardId` used by the reused,
  unmodified `InvoiceGenerationService`/`RateCardResolver` machinery: see the
  archived change's `lib/Service/TimeIntakeService.php` class docblock for
  the documented rate-resolution decision and its one known limitation
  (a batch without a request-level `rateCardId` bills at its first entry's
  resolved rate at the aggregate-invoice level; every entry's own resolved
  rate is still preserved on `UrenRegistratie.recognisedRate`).
