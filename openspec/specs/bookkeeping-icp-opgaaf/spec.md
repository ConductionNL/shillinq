...
### REQ-ICP-001: VIES validation SHALL be performed at invoice creation time for B2B intra-community supplies
...
### REQ-ICP-002: Periodicity SHALL auto-switch from quarterly to monthly when cumulative goods supplies exceed EUR 50,000 in any calendar quarter
...
### REQ-ICP-003: IcpSupply records SHALL aggregate by buyerVatId and supplyType, one line per unique combination per period
...
### REQ-ICP-004: IcpOpgaaf finalization SHALL reconcile total against BTW-aangifte rubriek 3b within EUR 1 tolerance
...
### REQ-ICP-005: SBR/Digipoort XML generation SHALL conform to `bd-rpt-icp-2026.xsd` (NT18 taxonomy) and be deliverable via existing Digipoort channel
...
### REQ-ICP-006: Triangulation transactions (ABC-levering) SHALL be reported on a separate line from regular goods and services
...
### REQ-ICP-007: Invoice PDF representation of ICP-treated supplies SHALL include reverse-charge notice and buyer VAT-ID
...
### REQ-ICP-008: Corrections to submitted IcpOpgaaf records SHALL be filed as separate "correction" filings, never in-place amendments
...
### REQ-ICP-009: VIES outage fallback and scheduled revalidation SHALL distinguish transient outages from definitive rejections
...
### REQ-ICP-010: Audit-trail export for Belastingdienst inspection SHALL produce a self-contained bundle in under 10 seconds
