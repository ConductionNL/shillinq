...
### BankStatement
**Schema.org:** `schema:Report`
_Bank statement header per import upload (CAMT.053 XML, MT940, or CSV) or PSD2 polling. Covers a bank account and period. The reconciliation lifecycle (`imported → in-progress → reconciled → audit-locked`) is declared as `x-openregister-lifecycle` per REQ-BR-008. Original file archived via docudesk (sourceDocumentUri). New-statement notifications declared as `x-openregister-notifications` — no BankNotificationService or MatcherService._
**Primary spec:** bookkeeping-bank-reconciliation (T2); also referenced by bookkeeping-bank-connectors (T4 PSD2 source)

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| bankConnectionId | string | No | FK to the BankConnection that sourced this statement (T4 PSD2 path) |
| statementFormat | enum | No | CAMT XML format version (T4 PSD2 path) |
| statementDate | date-time | No | Statement generation timestamp (T4 PSD2 path) |
| statementAttachmentUri | string | No | docudesk attachment URI for the CAMT.053 XML (T4 PSD2 path) |
| transactionCount | integer | No | Number of transactions in the statement (T4 PSD2 path) |
| importedAt | date-time | No | ISO 8601 timestamp of the operator import |
| importedBy | string | No | Nextcloud user ID of the importing operator |
| format | enum | Yes | Import format: camt.053, mt940, csv |
| fileChecksum | string | No | SHA-256 checksum for duplicate-import detection per REQ-BR-009 |
| periodStart | date | Yes | First date of the statement period |
| periodEnd | date | Yes | Last date of the statement period |
| openingBalance | number | No | Bank account opening balance |
| closingBalance | number | No | Bank account closing balance |
| currency | string | Yes | ISO 4217 currency code |
| bankAccountIban | string | No | IBAN of the bank account |
| bankAccountName | string | No | Account holder name |
| sourceDocumentUri | string | No | docudesk URI of the archived original file per ADR-022 |
| state | enum | Yes | Reconciliation state: imported, in-progress, reconciled, audit-locked |
| lineCount | integer | No | Total BankStatementLine count |
| matchedCount | integer | No | Lines confirmed-matched or routed-to-suspense |
| administrationId | string | Yes | FK to the Administration owning this statement |

**Relations:**
- → BankConnection (many-to-one, via bankConnectionId, T4 PSD2 path)
- → BankStatementLine (one-to-many, via statementId)

> **Reconciliation note (add-shillinq-bank-reconciliation, 2026-06-03):** BankStatement is additively extended from the T4 bank-connectors entry with full REQ-BR-002 fields, the `imported → in-progress → reconciled → audit-locked` lifecycle, and the duplicate-import uniqueness constraint on `administrationId + fileChecksum` / overlapping period per REQ-BR-009. The T4 PSD2-path fields (bankConnectionId, statementFormat, statementDate, statementAttachmentUri, transactionCount) are retained as optional for backwards compatibility. No PHP MatcherService or duplicate-detection service — all declarative per ADR-031.

### BankStatementLine
**Schema.org:** `schema:MonetaryAmount`
_A single transaction row in a bank statement. Created during import for each CAMT.053 entry, MT940 :61: block, or CSV data row. Carries the matching state and lifecycle (`unmatched → candidate → confirmed | routed-to-suspense`). Amount sign convention: positive = credit (incoming), negative = debit (outgoing) per CAMT convention (REQ-BR-003). x-openregister-audit: true._
**Primary spec:** bookkeeping-bank-reconciliation

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| statementId | string | Yes | FK to the parent BankStatement |
| lineNumber | integer | Yes | Sequential line number within the statement (1-based) |
| valueDate | date | Yes | Value date of the bank transaction |
| amount | number | Yes | Transaction amount (positive=credit, negative=debit per CAMT convention) |
| currency | string | Yes | ISO 4217 currency code |
| counterpartyName | string | No | Counterparty name as reported |
| counterpartyIban | string | No | Counterparty IBAN if present |
| reference | string | No | Payment reference (e.g. invoice number) |
| narrative | string | No | Free-text transaction description |
| rawPayload | string | No | Raw XML/text snippet from source statement for audit |
| matchState | enum | Yes | Current match state: unmatched, candidate, confirmed, routed-to-suspense |
| administrationId | string | Yes | FK to the Administration (inherited from parent statement) |

**Relations:**
- → BankStatement (many-to-one, via statementId)
- → ReconciliationMatch (one-to-many, via bankLineRefs)

### MatchingRule
**Schema.org:** `schema:Rule`
_Operator-authored rule declaring predicates for auto-matching bank statement lines against AP/AR invoices or GL transactions. Predicate list is consumed by the `candidateMatches` x-openregister-aggregations query (REQ-BR-004). No MatchingService.php or RuleEngine.php — declarative per ADR-031. Predicates are AND-combined within a rule; rules are evaluated in priority order (lower value first) per REQ-BR-004._
**Primary spec:** bookkeeping-bank-reconciliation

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| ruleName | string | Yes | Human-readable name for this rule |
| priority | integer | Yes | Evaluation priority (lower = earlier); default 100 |
| targetType | enum | Yes | Target schema: ap-invoice, ar-invoice, gl-transaction, customer, vendor |
| predicates | array | Yes | List of predicate objects (op + parameters per REQ-BR-005) |
| autoConfirm | boolean | Yes | If true, matches auto-confirmed without operator review |
| administrationId | string | Yes | FK to the Administration owning this rule |
| lifecycleState | enum | Yes | Rule state: active, disabled, archived |
| description | string | No | Operator-authored description |
| confidenceScore | number | No | Confidence score (0.0-1.0) for matches; default 1.0 |

**Relations:**
- → ReconciliationMatch (one-to-many, via matchingRuleId)

### ReconciliationMatch
**Schema.org:** `schema:Action`
_A produced match record linking one or more BankStatementLines (N-side) to one or more AP/AR invoices or GL transactions (M-side). Created by the `candidateMatches` aggregation. Confirmed or rejected by the operator or auto-confirmed via `MatchingRule.autoConfirm`. Confirmed matches emit a CloudEvent (`shillinq.reconciliation.match.confirmed`) consumed by AP/AR lifecycles per REQ-BR-006. x-openregister-audit: true._
**Primary spec:** bookkeeping-bank-reconciliation

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| bankLineRefs | array<string> | Yes | One or more BankStatementLine IDs (N-side) per REQ-BR-006 |
| targetType | enum | Yes | Target schema type: ap-invoice, ar-invoice, gl-transaction |
| targetRefs | array<string> | Yes | One or more target object IDs (M-side) per REQ-BR-006 |
| confidence | number | Yes | Confidence score (0.0-1.0) from the producing matching rule |
| isPartial | boolean | No | True if bank line amount < total target amount (partial payment) |
| state | enum | Yes | Match lifecycle state: candidate, confirmed, rejected |
| matchingRuleId | string | No | FK to the MatchingRule that produced this candidate |
| confirmedBy | string | No | Nextcloud user ID of the confirming operator |
| confirmedAt | date-time | No | ISO 8601 confirmation timestamp |
| rejectedReason | string | No | Operator reason for rejection |
| administrationId | string | Yes | FK to the Administration owning this match |

**Relations:**
- → MatchingRule (many-to-one, via matchingRuleId)
- → BankStatementLine (many-to-many, via bankLineRefs)
