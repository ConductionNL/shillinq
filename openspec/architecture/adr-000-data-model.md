...
### CommercialActivity / IntegralCostPrice / ActivityCostAllocation (Wet Markt en Overheid)
**Schema.org:** `schema:Service` (CommercialActivity), `schema:PriceSpecification` (IntegralCostPrice), `schema:MonetaryAmount` (ActivityCostAllocation)
_WMO compliance data model introduced by the `bookkeeping-market-government-separation` change (Phase 1 MVP). A `CommercialActivity` is the central register record for any market-facing activity a Dutch bestuursorgaan operates under the Mededingingswet hfd. 4b (gedragsregels Art. 25i–25l); it is a first-class record — not a free-form GL tag — so ACM meldingstatus, kostprijsmethode, kostenplaats/kostendrager coupling, marktcontext and the optional algemeen-belang-besluit exemption are all queryable (design D1). A customer/afnemer is a Nextcloud contact entity, not a bespoke schema, so `afnemers[]` holds external references only. The `IntegralCostPrice` is a time-versioned, immutable record per (activity, periode): each month produces a `voorlopig` record and the year-end (31-Mar) close produces a signed `definitief` record while the monthly records stay immutable for audit (design D2). Its six statutory components (directe loonkosten, directe materialen, directe afschrijvingen, indirecte overhead via the BBV taakveld 0.4 sleutel, vermogenskosten via WACC, winstopslag) are computed in integer cents and the Art. 25i compliance flag is derived from gehanteerdTarief ≥ kostprijsPerEenheid. The `ActivityCostAllocation` is a reversible split record describing how a journal-entry line touching an activity is divided across publieke/commerciële sub-administraties per the geldende `AllocationRule` (OverheadDistributionRule); the journal entry itself is never mutated, the split is derived (design D4), and a handmatige override needs verplichte motivering plus 4-ogen-akkoord. All three are schema-declared register-fragment metadata (ADR-037 fragment `register.d/bookkeeping-market-government-separation.json` with archived seed objects). The only PHP is the pure-logic, side-effect-free `lib/Service/WmoComplianceCalculator.php` (IKP composition, transaction split with cent-conservation, kostendekkingsratio, stale-review trigger) — no bespoke orchestration service ships (ADR-031), mirroring `TrialBalanceCalculator`. The overhead component deliberately reuses the BBV sleutel so WMO overhead equals BBV overhead, a critical consistency control (design D3). Phase 2 (ABB lifecycle, ACM reporting, cross-subsidy detection, immutable audit trail) and Phase 3 (activity transitions, governance coupling, multi-bestuursorgaan, market benchmarks) are deferred per the change tasks._
**Primary spec:** bookkeeping-market-government-separation

| Entity | Schema.org | Key fields | Primary relations |
|--------|-----------|-----------|-------------------|
| CommercialActivity | schema:Service | administrationId, code, naam, bestuursorgaan, marktsegment, concurrenten[], kostprijsMethode, kostenplaatsCode, kostendragerCode, isExempted, exemptionBesluitId, acmMelding, lastReviewedAt | → CostCenter (N:1 via kostenplaatsCode), → KostenDrager (N:1 via kostendragerCode), → AlgemeenBelangBesluit (N:1, Phase 2) |
| IntegralCostPrice | schema:PriceSpecification | administrationId, commercialActivityId, periode, status (voorlopig/definitief), componenten{6}, totaleKosten, kostprijsPerEenheid, gehanteerdTarief, marge, compliant | → CommercialActivity (N:1) |
| ActivityCostAllocation | schema:MonetaryAmount | administrationId, journalEntryId, commercialActivityId, originalAmount, splits[], verdeelsleutel, automatischToegepast, status, handmatigeOverride | → CommercialActivity (N:1), → AllocationRule (N:1 via verdeelsleutel), → GLLine (N:1 via journalEntryId) |
