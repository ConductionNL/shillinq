...
### DBAOpdracht / DBAIntake / DBAModelovereenkomst / DBARisicoflag / DBAPortfolioRisico / DBAEvidenceDossier
**Schema.org:** `schema:Order` (DBAOpdracht), `schema:Questionnaire` (DBAIntake), `schema:CreativeWork` (DBAModelovereenkomst), `schema:AlertAction` (DBARisicoflag), `schema:Report` (DBAPortfolioRisico), `schema:Collection` (DBAEvidenceDossier)
_DBA compliance marker data model introduced by the `dba-compliance-marker` change (T2). Operationalises the Wet DBA (deregulering beoordeling arbeidsrelaties, mei 2016) three-pijler regime (gezagsverhouding, persoonlijke arbeid, financieel risico), the Deliveroo-arrest criteria (HR 24-3-2023, ECLI:NL:HR:2023:443) and the upcoming VBAR uurtarief-rechtsvermoeden (EUR 33, peil 2024) into six declarative registers. `DBAOpdracht` is the per-engagement root (per klant/project/doorlopende relatie): it carries the intake reference, gekozen modelovereenkomst, lopende actueleRisicoscore + afgeleid risicoNiveau, openFlags count and evidenceDossier reference, plus a draft → actief → beeindigd lifecycle. The intake is deferred until the first factuur and enforced there (REQ-DBA-001); beeindiging starts the 7-jaars bewaartermijn (art. 52 AWR, REQ-DBA-018). `DBAIntake` stores the questionnaire answers structured in the three pijler subtotals (0-20 each) + Deliveroo subtotal (0-40); totaalScore is the declarative sum of the four subtotals (REQ-DBA-003), with a draft → submitted → completed lifecycle. `DBAModelovereenkomst` is the curated register of Belastingdienst-approved templates (tussenkomstvrij v3, leverancier-zelfstandig v2) + operator variants, each with essential bepalingen, version and geldigTot; expired models raise MODELOVEREENKOMST_VERLOPEN (REQ-DBA-002). `DBARisicoflag` is an immutable, append-only audit record raised by the monitoring engine (factuurfrequentie, concentratie, langjarigheid, VBAR-grens, vervangbaarheid, multiple-engagement-zelfde-concern), each citing its fiscale grondslag + actie-suggestie (REQ-DBA-004/005/006). `DBAPortfolioRisico` is the periodic aggregation across all active opdrachten of one onderneming (omzetconcentratie 12-maands-rolling, langjarige relaties, exclusiviteit) with an overallRisico band (REQ-DBA-005). `DBAEvidenceDossier` is the per-opdracht stukkenlijst (getekende overeenkomst, eerste/laatste factuur, urenstaten, opt-in e-mail-archief) with file-refs + SHA-256 hashes and a compleetheidScore (0-1), bound to the 7-jaars bewaartermijn (REQ-DBA-007/012/018). All six are schema-declared register-fragment metadata (ADR-037 fragment `register.d/dba-compliance-marker.json`); the centre of mass is declarative (x-openregister-lifecycle drives the state machines, x-openregister-calculations documents totaalScore + compleetheidScore, x-openregister-aggregations documents the portfolio omzetconcentratie roll-up). The only PHP exception-path code is `lib/Lifecycle/DBAComplianceGuard.php` (ADR-031) — it derives the four risk bands from the total score, gates the first factuur on a VOLTOOID intake, computes the VBAR effective-rate breach and the evidence completeness ratio, and gates beeindiging on a feitelijkeEindDatum. Object reads use the real OpenRegister ObjectService API only (ADR-022). A klant/opdrachtgever is a Nextcloud-addressbook-synced contact, referenced by klantId — never a bespoke person schema._
**Primary spec:** dba-compliance-marker

| Entity | Schema.org | Key fields | Primary relations |
|--------|-----------|-----------|-------------------|
| DBAOpdracht | schema:Order | ondernemingId, klantId, opdrachtNaam, startDatum, modelOvereenkomstId, intakeStatus, actueleRisicoscore, risicoNiveau, openFlags, evidenceDossierId, state, administrationId | → DBAIntake (1:1), → DBAModelovereenkomst (N:1), → DBAEvidenceDossier (1:1), → DBARisicoflag (1:N), → contact (klantId, N:1) |
| DBAIntake | schema:Questionnaire | opdrachtId, gezag/arbeid/financieel/deliverooSubtotaal, totaalScore, interpretatie, verkort, state, administrationId | → DBAOpdracht (1:1) |
| DBAModelovereenkomst | schema:CreativeWork | naam, bron, publicatieURL, goedkeuringDatum, geldigTot, essentieleBepalingen[], versie, actueleVersie | → DBAOpdracht (1:N) |
| DBARisicoflag | schema:AlertAction | opdrachtId, type, detectieMoment, ernst, details, fiscaleBron, actieSuggestie, status, administrationId | → DBAOpdracht (N:1) |
| DBAPortfolioRisico | schema:Report | ondernemingId, peilDatum, actieveOpdrachten, concentratie{}, langjarigeRelaties[], exclusieveRelaties, overallRisico, administrationId | → onderneming (N:1) |
| DBAEvidenceDossier | schema:Collection | opdrachtId, stukken[] (type/fileRef/datum/sha256), emailArchiefOptIn, compleetheidScore, ontbrekendeStukken[], bewaarTermijn, archiveDate, administrationId | → DBAOpdracht (1:1) |
