 ...
 * rules (mapping, passthrough, hex). This is the single ≤50 LOC service
 * permitted by the inventory-barcode-sku design (D2 / Declarative-vs-Imperative
 * decision): the declarative OpenRegister engine cannot interpolate a pattern
 * string with per-field rule transforms, so generation lives here.
 *
 * ADR-031 exception reason: pattern interpolation with field-mapping transforms
 * is procedural string assembly that the OR schema/lifecycle DSL cannot express.
 * Templates remain fully declarative (JSON); only the interpolation engine is PHP.
 * Replace with a declarative engine when OR gains pattern-interpolation support.
 *
 * @category Service
...
/**
 * Declarative SKU generator (ADR-031 exception per inventory-barcode-sku design D2).
 *
...
class SkuGenerator
{
     ...
     *
     * Reads the template from sku-templates.json, applies each rule's transform to
     * the matching product attribute, then interpolates the results into the
     * template pattern (e.g. "{category}-{manufacturer}-{size}-{color}").
     *
     * @param array<string, mixed> $item       Product (InventoryItem) attribute map,
     ...
     *
     * Supported types: 'mapping' (lookup table), 'passthrough' (verbatim),
    ...
    private function findTemplate(string $templateId): array
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE || is_array($data) === false) {
            throw new InvalidArgumentException('Failed to parse SKU template file: '.json_last_error_msg());
        }
