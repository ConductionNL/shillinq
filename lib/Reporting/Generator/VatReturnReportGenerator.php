...
final class VatReturnReportGenerator implements ReportGeneratorInterface {
	...
	public function generate(array $context, string $format): GeneratedFile {
		...
		$rubriek = $this->deriveFromInvoices($context);
		...
		if ($filing !== null) {
			...
			$rubriek['5b'] = $this->toFloat($filing['deductibleVat'] ?? $rubriek['5b']);
			$rubriek['3b'] = $this->toFloat($filing['intraCommunitySupplies'] ?? $rubriek['3b']);
		}

		$payable = ($rubriek['5a'] - $rubriek['5b']);
		...
		$this->writeRubriek($writer, '1a', 'Leveringen/diensten belast met hoog tarief', $rubriek['1a_base'], $rubriek['1a_vat']);
		// Rubriek 1b — leveringen/diensten belast met laag tarief (9%).
		$this->writeRubriek($writer, '1b', 'Leveringen/diensten belast met laag tarief', $rubriek['1b_base'], $rubriek['1b_vat']);
		// Rubriek 3b — intracommunautaire leveringen.
		$this->writeRubriek($writer, '3b', 'Leveringen naar landen binnen de EU', $rubriek['3b'], 0.0);
		// Rubriek 5a — verschuldigde omzetbelasting (totaal).
		$this->writeRubriek($writer, '5a', 'Verschuldigde omzetbelasting', null, $rubriek['5a']);
		// Rubriek 5b — voorbelasting.
		$this->writeRubriek($writer, '5b', 'Voorbelasting', null, $rubriek['5b']);
