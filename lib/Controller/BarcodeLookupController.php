 ...
 * record plus the expanded InventoryItem product data, returning the per-UoM
 * quantity so the POS can offer "add 1 carton (= N units)".
 *
 * @category Controller
...
/**
 * Barcode lookup endpoint for POS scanning per REQ-SKU-007/008.
 *
 * Authentication (ADR-005): the endpoint is PublicPage so provisioned POS
 * terminals (which hold no Nextcloud session) can call it, but every request
 * MUST present a valid Bearer API key matching the app config secret
 ...
 * rejects anonymous callers (never an open endpoint). The API key is read from
 * app config and never returned in any response.
 *
 * @spec openspec/changes/inventory-barcode-sku/tasks.md#task-12
 */
class BarcodeLookupController extends Controller
{
     ...
     *
     * Returns 401 when the Bearer API key is missing/invalid (and no NC session
     * fallback applies), 404 when no active barcode matches, 200 with the
     * barcode + product envelope otherwise. Inactive barcodes (isActive=false)
     * are never returned per REQ-SKU-008.
     *
     ...
     *
     * When an API key is configured, the request MUST carry a matching Bearer
     * token (hash_equals, constant-time). When no API key is configured, fall
     * back to requiring an authenticated Nextcloud user — anonymous callers are
     * always rejected.
     *
    ...
    {
        $configuredKey = $this->appConfig->getValueString(Application::APP_ID, self::API_KEY_CONFIG, '');
    ...
    private function findActiveBarcode(string $code, ?string $uomCode): ?array
    {
        ...
        foreach ($this->findMany(schema: 'Barcode', filters: $filters) as $record) {
            // REQ-SKU-008: inactive barcodes are never returned. isActive defaults
    ...
    /**
     * Resolve the related InventoryItem product for the response envelope.
     *
     * Returns null when inventory-product-catalog has not yet seeded the
     * InventoryItem register (the barcode is still resolvable on its own).
     *
    ...
    private function findProduct(string $sku): ?array
    {
        ...
        $items = $this->findMany(schema: 'InventoryItem', filters: ['sku' => $sku], limit: 1);
        if (count($items) === 0) {
            return null;
        }
